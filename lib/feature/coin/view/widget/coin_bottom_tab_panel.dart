...
class CoinBottomTabPanel extends StatelessWidget {
        ...
        children: [
          ...
          Expanded(
            child: selectedBottomTab == 'holding'
                ? _buildHoldingTable(isCompact: isCompact)
                : _buildTradeHistoryTable(isCompact: isCompact),
          ),
        ],
      ),
    );
  ...
  Widget _buildHoldingTable({
    ...
    return Column(
      children: [
        _buildHoldingHeaderRow(isCompact: isCompact),
        Expanded(
    ...
    return Container(
      ...
      child: Row(
        children: [
          const Expanded(
            flex: 3,
            child: _TableHeaderText('코인'),
          ),
          const Expanded(
            flex: 2,
            child: _TableHeaderText('수량', alignRight: true),
          ),
          if (!isCompact)
            const Expanded(
              flex: 2,
              child: _TableHeaderText('평균가', alignRight: true),
            ),
          const Expanded(
            flex: 2,
            child: _TableHeaderText('평가금액', alignRight: true),
          ),
          const Expanded(
            flex: 2,
            child: _TableHeaderText('손익', alignRight: true),
          ),
        ],
      ),
    );
  ...
  Widget _buildHoldingRow({
    required CoinHoldingModel holding,
    required bool isCompact,
  }) {
  ...
  Widget _buildTradeHistoryTable({
    ...
    return Column(
      children: [
        _buildTradeHistoryHeaderRow(isCompact: isCompact),
        Expanded(
          child: ListView.separated(
            padding: EdgeInsets.zero,
            physics: const ClampingScrollPhysics(),
      ...
      child: Row(
        children: [
          ...
          if (!isCompact)
            const Expanded(
              flex: 2,
              child: _TableHeaderText('가격', alignRight: true),
            ),
          const Expanded(
            flex: 2,
            child: _TableHeaderText('금액', alignRight: true),
          ),
          if (!isCompact)
            const Expanded(
              flex: 2,
              child: _TableHeaderText('수수료', alignRight: true),
            ),
        ],
      ),
    );
  ...
  Widget _buildTradeHistoryRow({
    required CoinTradeHistoryModel history,
    required bool isCompact,
  }) {
        ...
        children: [
          ...
          if (!isCompact)
            Expanded(
              flex: 2,
              child: Text(
                moneyFormat.format(history.fee),
                textAlign: TextAlign.right,
                style: _tableTextStyle(),
              ),
            ),
